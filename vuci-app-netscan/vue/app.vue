  ...
  <div class="form">
    <a-form :form="form" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }" @submit="handleSubmit">
      ...
      <a-form-item label="Netmask size">
        <a-input
          v-decorator="['netmask', { initialValue: netmask, rules: [{ required: true, validator: validateSubnet }] }]"
        />
      </a-form-item>
    ...
    <div v-if="networks.length !== 0" class="topology-container" ref="topology">
      ...
      <div class="topology-unknown">
        <div v-for="(network, idx) in networks.unknown" :key="idx">
          <host ref="unknown" :network="network" :hostnames="scanHostnames"/>
        </div>
      </div>
      <hr>
      <div class="topology-known">
        <div v-for="(network, idx) in networks.known" :key="idx">
          <host ref="known" :network="network" :hostnames="scanHostnames"/>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    validateSubnet (rule, value, message) {
      if (value == null || value === '') {
        message('Field cannot be empty!')
        return
      }
      ...
      message('Netmask size is invalid! Must be between 1 - 31')
    },
        ...
        .then(response => {
          if (response.ok) {
            try {
              ...
              this.$message.error(response.data[0], 5)
            }
            this.$timer.stop('getScanResults')
            this.loading = false
          }
