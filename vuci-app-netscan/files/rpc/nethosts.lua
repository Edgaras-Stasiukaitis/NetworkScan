...
parser:option("-i --ipsubnet", "Specific ipv4 subnet to scan")
parser:option("-r, --range", "Port range to scan")
...
NetworkDatabase.KnownHosts = {
    {   macAddr = "00:1e:42:24:d7:17",
        description = "My router",
        vendor = "Teltonika RUTX11",
    },
}
    ...
    for _, ThisNetworkHost in ipairs( HostsFoundOnNetwork ) do
        ...
        if args.ports then
            ...
            if args.range ~= nil then
                cmd = cmd .. "-p " .. args.range .. " "
            else
                cmd = cmd .. "-F "
            end
            cmd = cmd ..ThisNetworkHost.ipNumber.." | grep 'open' | awk '{ print $1 }' | awk -F/ '{print \""..ThisNetworkHost.ipNumber.." \" $1}' >> address_ports"
            os.execute(cmd)
        end
...
function printHostReportRecord ( familiarityTag, NetworkHost )
    ...
    local jsonFormat = "{\"host\":\"%s\",\"ipNumber\":\"%s\",\"macAddress\":\"%s\",\"vendor\":\"%s\",\"ports\":[%s]}"
    if args.ports then
        local ports = Group_ports_by_address()
        local portsText = ""
        if ports[ipNumberString] ~= nil then
            portsText = table.concat(ports[ipNumberString], ",")
        end
        io.write(string.format(jsonFormat, familiarityTag, ipNumberString, macAddrString, vendor, portsText))
        return
    end
    -- Use the provided format string to print this host record.
    io.write(string.format(jsonFormat, familiarityTag, ipNumberString, macAddrString, vendor, ""))
...
function Group_ports_by_address()
    local address_ports = {}
